function [Cost_new,Cost,fNorm,tNorm,sNorm] = Cost_STCR(fUpdate, Image, para, Cost_old)

fNorm = sum(abs(fUpdate(:)).^2);
Image = crop_half_FOV(Image);
if para.Recon.weight_tTV ~= 0
    tNorm = para.Recon.weight_sTV .* abs(diff(Image,1,3));
    tNorm = sum(tNorm(:));
else
    tNorm = 0;
end
...
if para.Recon.weight_sTV ~= 0
    ...
    sy_norm(end+1,:,:,:,:)=0;
    sNorm = para.Recon.weight_sTV .* sqrt(abs(sx_norm).^2+abs(sy_norm).^2);
    sNorm = sum(sNorm(:));
else
    sNorm = 0;
end
